...
var load_engine = (function () {
  ...
  function new_worker(path, options) {
    ...
    if (
      ...
      return spawn_worker(
        path || require("path").join(__dirname, "src", "stockfish.js"),
        options
      );
    }
  ...
  return function load_engine(path, options) {
    ...
    function determine_que_num(line, que) {
      ...
      if (que[0].cmd !== "bench" && que[0].cmd !== "perft") {
        ...
        for (i = 0; i < len; i += 1) {
          ...
          if (
            cmd_first_word === cmd_type ||
            (cmd_type === "other" &&
              (cmd_first_word === "d" || cmd_first_word === "eval"))
          ) {
            return i;
          }
        }
    ...
    worker.onmessage = function onmessage(e) {
      ...
      if (
        !que.length ||
        line.substr(0, 14) === "No such option" ||
        line.substr(0, 3) === "id " ||
        line.substr(0, 9) === "Stockfish"
      ) {
        return;
      }
      ...
      if (line === "uciok") {
        ...
        if (
          line.substr(0, 15) === "Legal uci moves" ||
          line.substr(0, 6) === "Key is"
        ) {
          my_que.done = true;
          done = true;
